...
public class KeywordDictionary {
	...
	public void Initialize() {
		...
		for(tnames t : tnames.values()) dict.put(t.getLexeme(), t);
	}
	
	public Token getToken(String s) {
		//If string corresponds to a lexeme in the dictionary, return corresponding token 
		tnames ret = getKey(s);
		
		//If string does not correspond to a lexeme but is valid name, return identifier
		if(ret==null && SourceVersion.isName(s)) {
			if(Character.isUpperCase(s.charAt(0))) return new Token(tnames.TYPEID, s);
