...
public enum pnames implements names {
	...
	EXPR_(new names[][] {
			{tnames.ASSIGN, pnames.EXPR, tnames.SEMI},
			{tnames.SEMI}
	}),
	FEATURE(new names[][] {
			{tnames.BRACKETOPEN, pnames.FORMAL, tnames.BRACKETCLOSE, tnames.COLON, tnames.TYPEID, tnames.BRACEOPEN, pnames.EXPR, tnames.BRACECLOSE, tnames.SEMI, pnames.PH_FEATUREDEF},
			{tnames.COLON, tnames.TYPEID, pnames.EXPR_, pnames.PH_FEATUREDEF}
	}),
	FEATUREDEF(new names[][] {
			{tnames.ID, pnames.FEATURE},
			{tnames.EPSILON}
	}),
	...
	CLASSDEF(new names[][] {
			{tnames.CLASS, tnames.TYPEID, pnames.IHERITANCE, tnames.BRACEOPEN, pnames.FEATUREDEF, tnames.BRACECLOSE}
	}),
	CLASSDEF_(new names[][] {
			{pnames.PH_PROGRAM},
			{tnames.EPSILON}
	...
	public void fixparts() {
		for(names[] p : this.parts) {
			for(int i=0; i<p.length; i++) {
				...
				else if(p[i]==pnames.PH_FEATUREDEF) p[i]=pnames.FEATUREDEF;
